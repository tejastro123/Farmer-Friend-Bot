...
const TempImgPage = () => {
  ...
  const MAP_LAYERS = {
    ...
    esri: { name: "Esri Street", url: "https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}", attribution: 'Esri' },
    topo: { name: "Topo Map", url: "https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png", attribution: 'OpenTopoMap' },
  };
  ...
  const handleDownload = (image) => {
    const downloadUrl = image.download_url || `https://planet.com/data/v1/download?id=${image.id}`;
                ...
                {imagery.map((img, idx) => (
                  ...
                  >
                    <div className="card-preview">
                      {img.thumbnail ? (
                        <img src={img.thumbnail} alt="Thumbnail" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                      ) : null}
                      <div className="card-placeholder" style={{ display: img.thumbnail ? 'none' : 'flex' }}>
                        <Layers size={24} />
                      </div>
                    </div>
                    <div className="card-info">
                      <span className="card-date">{img.date || "Unknown"}</span>
                      <span 
          ...
          <div className="preview-content" onClick={(e) => { e.stopPropagation(); }}>
            ...
            <div className="preview-image">
              {previewImage.preview ? (
                <img src={previewImage.preview} alt="Satellite Preview" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
              ) : null}
              <div className="preview-placeholder" style={{ display: previewImage.preview ? 'none' : 'flex' }}>
                <Layers size={64} />
                <p>Satellite Image Preview</p>
                <p className="hint">{isDemo ? "Demo data - no real imagery" : "Loading preview..."}</p>
              </div>
            </div>
